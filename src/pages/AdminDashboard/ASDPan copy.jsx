...
const ASDPan = () => {
  ...
  const downloadDocument = async (documentPath, documentName) => {
    try {
      console.log("Downloading from:", documentPath);
  ...
  if (error)
    return (
      <ErrorContainer>
        <ErrorIcon />
        <ErrorText>Error fetching service details</ErrorText>
      </ErrorContainer>
    );
...
// Styled Components (Add these to the existing styled components)
...
// Styled Components (Simplified for readability)
...
// Basic Loading and Error Components (could be extracted to a separate file)
