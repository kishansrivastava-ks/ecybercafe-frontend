...
const fetchJobCardServices = async () => {
  ...
  // Filter only JobCard services
  return res.data.filter((service) => service.serviceType === "JobCard");
};

const JobCardServices = () => {
  const { data, error, isLoading } = useQuery({
    queryKey: ["jobCardServices"],
    queryFn: fetchJobCardServices,
  });
  ...
  const fetchServiceDetails = async (serviceId) => {
    ...
    // Navigate to JobCard details page
  ...
  const deleteServiceMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["jobCardServices"]);
      successToast("JobCard service deleted successfully");
    },
    onError: () => {
      errorToast("Failed to delete JobCard service");
    },
  });

  const deleteAllServicesMutation = useMutation({
    mutationFn: async () => {
      // Get all JobCard service IDs and delete them
      const jobCardServices = data || [];
      const deletePromises = jobCardServices.map((service) =>
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["jobCardServices"]);
      successToast("All JobCard services deleted successfully");
    },
    onError: () => {
      errorToast("Failed to delete JobCard services");
    },
  });
  ...
  return (
    ...
    >
      <Title>My JobCard Services</Title>
      ...
      <DeleteAllContainer>
        ...
        >
          Delete All JobCard Services
          <Trash2 size={16} />
        </DeleteAllButton>
      </DeleteAllContainer>

      {isLoading && (
        <LoadingMessage>Loading JobCard services...</LoadingMessage>
      )}
      {error && <ErrorMessage>Error fetching JobCard services</ErrorMessage>}
      {!isLoading && !error && filteredServices.length === 0 && (
        <EmptyState>
          No JobCard services found matching the selected filters.
        </EmptyState>
      )}
      ...
      {deleteConfirmation && (
        <ConfirmationModal>
          <ConfirmationContent>
            <h3>Delete JobCard Service</h3>
            <p>
              Are you sure you want to delete this JobCard service? This action
              cannot be undone.
            </p>
            <ConfirmationActions>
      ...
      {showDeleteAllConfirmation && (
        <ConfirmationModal>
          <ConfirmationContent>
            <h3>Delete All JobCard Services</h3>
            <p>
              Are you sure you want to delete all your JobCard services? This
