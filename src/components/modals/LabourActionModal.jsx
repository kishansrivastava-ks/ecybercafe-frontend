...
const LabourActionModal = ({ service, type, onClose, onSuccess }) => {
  const [remark, setRemark] = useState("");

  const mutation = useMutation({
    mutationFn: async () => {
      // POST /api/admin/service/:id/labour/action
      const res = await axiosInstance.post(
        `/admin/service/${service._id}/labour/action`,
        {
          action: type,
          remark: remark,
        }
    ...
    <Overlay>
      <Modal>
        ...
        <Body>
          ...
          >
            Applicant: <strong>{service.specificService?.name}</strong>
            <br />
            App No:{" "}
            <strong>{service.specificService?.applicationNumber}</strong>
          </p>
...
export default LabourActionModal;

// Reuse styles for Overlay, Modal, etc. from AdminLabourServices or create new file
